...
export const Login: React.FC = () => {
  ...
  const [verificationCode, setVerificationCode] = useState(''); // The code user enters
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    const user = login(email, password, loginType);
    
    if (user) {
      if (user.role === 'admin' || user.role === 'publisher') {
          navigate('/admin');
      } else {
          navigate('/'); // Subscribers go to home
      }
    } else {
  ...
  // Forgot Password - Step 1: Find User & Generate Code
  const handleRecoveryStart = (e: React.FormEvent) => {
      e.preventDefault();
      setError('');
      
      const result = initiateRecovery(recoveryIdentifier);

      if (result) {
          // Simulation of sending email. We display the content in alert as requested.
  ...
  const handleRecoveryComplete = (e: React.FormEvent) => {
      ...
      const success = completeRecovery(recoveryIdentifier, verificationCode, newPassword);
      if (success) {
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-paper">
      <div className="max-w-md w-full bg-white shadow-2xl border-t-4 border-gold relative overflow-hidden transition-all duration-300">
        ...
        {/* Render Tabs ONLY if not recovering */}
        {!isRecovering && (
                ...
                <form className="mt-8 space-y-6" onSubmit={handleLoginSubmit}>
                    ...
                    <button
                        ...
                        ${loginType === 'admin' 
                            ...
                            : 'bg-gold hover:bg-gold-dark focus:ring-gold'}`}
                    >
